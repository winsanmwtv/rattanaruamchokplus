...
export async function POST(request) {
    try {
        ...
        const [stockRows] = await promisePool.query(
            `SELECT front_quantity, back_quantity FROM Product WHERE barcode = (SELECT barcode FROM Product WHERE barcode = ? LIMIT 1) AND branch_id = ? LIMIT 1`,
            [prod_id, branch_id]
        );
        ...
        if (frontQuantity >= quantityToDeduct) {
            ...
            frontQuantity = 0;
            if (backQuantity >= quantityToDeduct) {
                backQuantity -= quantityToDeduct;
                quantityToDeduct = 0;
            } else {
        ...
        await promisePool.query(
            `UPDATE Product SET front_quantity = ?, back_quantity = ? WHERE barcode = (SELECT barcode FROM Product WHERE barcode = ? LIMIT 1) AND branch_id = ?`,
