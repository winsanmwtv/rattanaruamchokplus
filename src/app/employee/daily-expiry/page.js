...
const paymentGateway = '0935182935';
...
const ExpiryCheckPage = () => {
    ...
    useEffect(() => {
        if (window.innerWidth <= 768) {
            router.push('/employee/daily-expiry/mobile-app');
            return;
        }
    ...
    const paymentMenuButtons = [
        { label: "ยืนยันการทำรายการ", action: handleCashPayment },
        { label: "เพิ่มรายการสินค้า", action: () => { setIsPaymentMode(false); setCashReceived(""); setErrorMessage(""); } }
    ];
...
export default ExpiryCheckPage;
